...
Draw.Line = Draw.extend({
  ...
  disable() {
    ...
    // fire drawend event
    this._map.fire('pm:drawend', { shape: this._shape ,mdsMeasueLayer:this._mdsMeasurementLayer});
      ...
      for (var i = 1, len = latLngs.length ; i < len; i++) {
          ...
          pixelDist = p1.distanceTo(p2);
            ...
            L.marker.measurement(
              ...
              mdsUtilsMesaure.formatDistance(dist), "mds--mds", mdsUtilsMesaure.mdsGetRotation(this._map, ll1, ll2), this.mdsOptions)
              .addTo(this._mdsMeasurementLayer);
          
      }

      let  latLngsLast = latLngs[latLngs.length-1];
      const mdsMidlelatlong = Utils.calcMiddleLatLng(this._map,latLngsLast,this._hintMarker.getLatLng());
    
      this.mdsLastLL = this._hintMarker.getLatLng();
      const dis = latLngsLast.distanceTo(this._hintMarker.getLatLng());
